...
def getSongsFromAlbum(URL):
    ...
    for i in range( ceil(trackNumber / trackLimit) ):
        ...
        for t in rawTracks:
            tracks.append({'trackName':t['name'], 'artist':t['artists'][0]['name'], 'search':f"{t['name']}{t['artists'][0]['name']}"})
    return tracks
...
def getSongFromTrack(URL):
    ...
    return [{'trackName':t['name'], 'artist':t['artists'][0]['name'], 'search':f"{t['name']} {t['artists'][0]['name']}"}]
